...
final readonly class BrowserToolset
{
    ...
    /** @return list<Tool> */
    public function forAttachment(string|object $owner, string $attachmentId): array
    {
        $navigate = (new Tool)->as('browser_navigate')
            ...
            ->using(fn (string $url): string => $this->guard->guard($this->browser->navigate($owner, $attachmentId, $url)));

        $observe = (new Tool)->as('browser_observe')
            ->for('Read a bounded structured observation of the current rendered page.')
            ->using(fn (): string => $this->guard->guard($this->browser->observe($owner, $attachmentId)));
        ...
        $act = (new Tool)->as('browser_act')
            ...
            ->using(function (string $kind, string $observation_id, string $ref, ?string $value = null) use ($owner, $attachmentId): string {
                $action = new BrowserAction(ActionKind::from($kind), $observation_id, $ref, $value);

                return $this->guard->guard($this->browser->act($owner, $attachmentId, $action));
            });

        $status = (new Tool)->as('browser_status')
            ->for('Report the browser attachment state and generation without exposing its checkpoint.')
            ->using(function () use ($owner, $attachmentId): string {
                $attachment = $this->browser->status($owner, $attachmentId);
        ...
        $close = (new Tool)->as('browser_close')
            ...
            ->requiresApproval()
            ->using(function () use ($owner, $attachmentId): string {
                $attachment = $this->browser->close($owner, $attachmentId);
